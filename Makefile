...
generate-percona-crd:
	GOBIN='$(CURDIR)/hack/tools' ./hack/controller-generator.sh \
		crd:crdVersions='v1' \
		paths='./pkg/apis/pgv2.agora.com/...' \
		output:dir='build/crd/percona/generated' # build/crd/generated/{group}_{plural}.yaml
	$(PGO_KUBE_CLIENT) kustomize ./build/crd/percona/ > ./config/crd/bases/pgv2.agora.com_perconapgclusters.yaml

generate-crd: generate-crunchy-crd generate-percona-crd
	cat ./config/crd/bases/pgv2.agora.com_perconapgclusters.yaml > ./deploy/crd.yaml
